...
def center_bead_3d_window (update_bead_reg, intensity_img, window_z=6, window_xy=6):
    """
    ...
    for bead, coor in update_bead_reg.items():
        ...
        std = 0
        center_cut = 0
        for slice in range (0, window_z*2):
...
print (prod_optical_control)
beads_output_folder = '/allen/aics/microscopy/PRODUCTION/OpticalControl/output/psf'
print (beads_output_folder)
df = pd.DataFrame()
...
# processed_folders = os.listdir(os.path.join(beads_output_folder, 'processed'))

folders = os.listdir(beads_output_folder)
reprocess = []
for folder in folders:
    if folder.startswith('ZSD'):
        files = os.listdir(os.path.join(beads_output_folder, folder))
        if len(files) == 0:
...
for optical_control_folder in optical_control_folders:
    ...
    if optical_control_folder in reprocess:
        print ('processing folder: ' + str(count) + ' out of ' + str(total))
        count += 1
                ...
                for channel in channels:
                    ...
                    if len(update_bead_reg) > 0:
                        ...
                        bead_3d_window = center_bead_3d_window(update_bead_reg=update_bead_reg, intensity_img=filtered,
                                                               window_z=window_z, window_xy=window_xy)
                    else:
                        bead_3d_window = None
                        failed_beads = failed_beads.append(row, ignore_index=True)
                    ...
                    if bead_3d_window is not None:
                        ...
                        writer_psf_3d = omeTifWriter.OmeTifWriter(os.path.join(beads_img_save_folder, file + '_psf3d.tif'))
                            ...
                            for x in range (int(img_w/box), img_w+1, int(img_w/box)):
                                ...
                                if flag >0 :
                                    ...
                                    for bead in beads_in_box:
                                        ...
                                        psf_3d_box[count, :, :, :] = bead_3d_window[bead]
                                        count+=1
                                    ...
                                    # Save psf_3d_box
                                    writer_psf_3d_box = omeTifWriter.OmeTifWriter(os.path.join(beads_img_save_folder, file + '_3d_box_' + box_id_name[box_id] + '.tif'))
                                    ...
                                    for axis in range (0, 3):
                                        ...
                                        writer_axis = omeTifWriter.OmeTifWriter(os.path.join(beads_img_save_folder, file + '_box_' + box_id_name[box_id] + '_' + axis_dict[axis] + '.tif'))
                        ...
                        '''
                            ...
                            print ('failed to process beads in img' + file_name + ' for system ' + system )
...
df.to_csv(os.path.join(beads_output_folder, 'psf_data_2.csv'))
failed_beads.to_csv(os.path.join(beads_output_folder, 'failed_beads_2.csv'))
failed_beads_fit.to_csv(os.path.join(beads_output_folder, 'failed_beads_fit_2.csv'))
