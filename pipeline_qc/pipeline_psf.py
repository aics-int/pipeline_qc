...
beads_img_save_folder = r'\\allen\aics\microscopy\Calysta\test\psf\20191031'
channels = beads.get_channel_names()
img = beads.data[0, channels.index('EGFP'), :, :, :]
...
with open (os.path.join(beads_img_save_folder, file + '_parameters_test.json'), 'w') as f:
    json.dump(par_output['bead_reg'], f)

df = df.append(row, ignore_index=True)
df.to_csv(os.path.join(beads_img_save_folder, 'psf_data.csv'))
