...
"""
# User's input
input_folder = r'\\allen\aics\microscopy\Antoine\drug_project\Segmentation_pre_production\Tom20\Organized dataset'
output_path = r'\\allen\aics\microscopy\Antoine\drug_project\Segmentation_pre_production\Tom20\QC'
...
try:
    os.mkdir(os.path.join(output_path, 'QC'))
except:
    pass

try:
    os.mkdir(os.path.join(output_path, 'QC', 'qc_images'))
except:
    pass
...
all_folders = [x[0] for x in os.walk(input_folder)]
for folder in all_folders:
    ...
    for img_file in images:
        ...
        if img_file.endswith('.czi'):
            ...
            img = AICSImage(os.path.join(folder, img_file))
            ...
            settings = (np.min(mip_xy_0), np.max(mip_xy_0))
            ...
            for key, image in new_images_dict.items():
                writer = omeTifWriter.OmeTifWriter(os.path.join(output_path, key, file_name + '-' + key + '.tif'),
                                                   overwrite_file=True)
